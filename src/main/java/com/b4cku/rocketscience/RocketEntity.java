...
public class RocketEntity extends ThrownItemEntity {
    ...
    public void tick() {
        ...
        /*if (rocket_age < ROCKET_AGE_NO_FUEL) {
            setVelocity(getVelocity().multiply(1f + 0.005f * fuelCheck()));
        }*/
        //i will implement this at some point to maintain the speed rather constant, but it's for another day

        if (rocket_age == ROCKET_AGE_BIGGER_EXPLOSION) {
    ...
    private float fuelCheck() {
        if (rocket_age < ROCKET_AGE_LOW_FUEL) {
            return 1f;
        }
        else if (rocket_age > ROCKET_AGE_NO_FUEL) {
            return 0f;
        }
        return 1 - ((float) (rocket_age - ROCKET_AGE_LOW_FUEL) / (ROCKET_AGE_NO_FUEL - ROCKET_AGE_LOW_FUEL));
    }
    ...
    private void blowUp() {
        ...
        this.getWorld().createExplosion(this, this.getX(), this.getY(), this.getZ(), rocket_age > ROCKET_AGE_BIGGER_EXPLOSION ? ROCKET_EXPLOSION_POWER_RIDDEN : ROCKET_EXPLOSION_POWER, false, World.ExplosionSourceType.NONE);
